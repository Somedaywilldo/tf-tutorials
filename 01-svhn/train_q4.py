...
def main():
    ...
    # loss = tf.losses.mean_pairwise_squared_error(label_onehot, logits) + loss_reg
    ...
    config.log_dir = args.log_dir + "_p%d"%(args.p)

    train_writer = tf.summary.FileWriter(os.path.join(config.log_dir+"_"+args.loss, 'tf_log', 'train'),
                                         tf.get_default_graph())
    test_writer = tf.summary.FileWriter(os.path.join(config.log_dir+"_"+args.loss, 'tf_log', 'test'),
                                        tf.get_default_graph())
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epoch_start+1, config.nr_epoch+1):
            ...
            if epoch % config.snapshot_interval == 0:
                saver.save(sess, os.path.join(config.log_model_dir+'_'+args.loss, 'epoch-{}'.format(epoch)),
