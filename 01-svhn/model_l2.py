...
class Model():
    ...
    def _pool_layer(self, name, inp, ksize, stride, padding='SAME', mode='MAX'):
        assert mode in ['MAX', 'AVG', 'L2'], 'the mode of pool must be MAX or AVG'
        if mode == 'MAX':
            ...
            x = tf.math.square(inp)
            x = tf.nn.avg_pool(x, ksize=[1, ksize, ksize, 1], strides=[1, stride, stride, 1],
                               padding=padding, name=name, data_format='NHWC')
            x = tf.math.sqrt(x)
        return x
    ...
    def build(self):
        ...
        x = self._pool_layer(name='pool1', inp=x, ksize=2, stride=2, mode='L2') # Nx16x16x16
        ...
        x = self._conv_layer(name='conv22', inp=x, kernel_shape=[3, 3, 32, 32],
                             stride=1, is_training=is_training)
        x = self._pool_layer(name='pool2', inp=x, ksize=2, stride=2, mode='L2') # Nx8x8x32
        ...
        x = self._conv_layer(name='conv32', inp=x, kernel_shape=[3, 3, 64, 64],
                             stride=1, is_training=is_training)
        x = self._pool_layer(name='pool3', inp=x, ksize=2, stride=2, mode='L2') # Nx4x4x64
        ...
        x = self._conv_layer(name='conv42', inp=x, kernel_shape=[3, 3, 128, 128],
                             stride=1, is_training=is_training)
        x = self._pool_layer(name='pool4', inp=x, ksize=4, stride=4, mode='L2') # Nx1x1x128
